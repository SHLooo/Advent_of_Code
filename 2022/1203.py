...
for rucksack in rucksacks:
    ...
    total_priority += (ord(item)-96 if ord(item)-96 >= 0 else ord(item)-64+26)
...
for i in range(0, len(rucksacks), 3):
    common_item = (set(rucksacks[i]) & set(rucksacks[i+1]) & set(rucksacks[i+2])).pop()
    total_priority += (ord(common_item)-96 if ord(common_item)-96 >= 0 else ord(common_item)-64+26)
print(total_priority)